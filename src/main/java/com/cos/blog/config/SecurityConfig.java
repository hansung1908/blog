...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    // 시큐리티가 대신 로그인해주는데 password를 가로채면
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable() // csrf 토큰 비활성화 (테스트시 걸어두는게 좋음)
                .authorizeRequests()
                .antMatchers("/", "/auth/**", "/js/**", "/css/**", "/image/**", "/dummy/**") // /auth로 시작하는 주소를
                .permitAll() // 다 허락
                .anyRequest() // 이외의 다른 요청들은
                .authenticated() // 인증 보안으로 막아
                .and() // 앞에거 한뒤에 그리고
                .formLogin() // 로그인 폼으로 이동
                .loginPage("/auth/loginForm") // 로그인 페이지 주소
                .loginProcessingUrl("/auth/loginProc") // 스프링 시큐리티가 해당 주소로 요청오는 로그인을 가로채서 대신 로그인한다.
                .defaultSuccessUrl("/"); // 인증 완료되면 이 주소로 ㄱㄱ
    }
}
